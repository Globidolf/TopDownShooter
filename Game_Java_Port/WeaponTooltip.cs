...
namespace Game_Java_Port {
    class WeaponTooltip : Tooltip {
        ...
        public override void draw(DeviceContext rt) {
            ...
            rt.DrawBitmap(dataLoader.get2D("icon_bg"), new RectangleF(temp.X, temp.Y + temp.Height - 20, 16, 16), 1, BitmapInterpolationMode.Linear);
            rt.DrawBitmap(dataLoader.get2D("Coin"), new RectangleF(temp.X, temp.Y + temp.Height - 20, 16,16), 1, BitmapInterpolationMode.Linear);
            temp.Offset(0, 20);
            rt.DrawBitmap(dataLoader.get2D("icon_bg"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            rt.DrawBitmap(dataLoader.get2D("Attack"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            temp.Offset(0, 20);
            rt.DrawBitmap(dataLoader.get2D("icon_bg"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            rt.DrawBitmap(dataLoader.get2D("Precision"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            temp.Offset(0, 20);
            rt.DrawBitmap(dataLoader.get2D("icon_bg"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            rt.DrawBitmap(dataLoader.get2D("Firerate"), new RectangleF(temp.X, temp.Y, 16, 16), 1, BitmapInterpolationMode.Linear);
            temp.Offset(0, 20);
            temp = relLabel;
            temp.Offset(20, 0);
