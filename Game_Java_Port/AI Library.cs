	...
	public static class AI_Library {
		...
		public static Action<NPC> RealPlayer = (me) =>
		{
            ...
            if(MousePos != _LastMousePos) {
				...
				me.AimDirection = Program.center.angleTo(new Vector2(MousePos.X / Program.form.ClientSize.Width * Program.width, MousePos.Y / Program.form.ClientSize.Height * Program.height));

			} else if(me.MovementVector.LengthSquared() != 0) {
				range = Math.Max(me.WeaponRangeR, me.MeleeRangeR);
				me.AimDirection = Program.center.angleTo(MousePos);
			}
		...
		public static Action<NPC> PlayerSim = (me) =>
				{
					...
					if(me.justPressed(Controls.open_pausemenu))
						GameMenu.MenuList[MenuName.PauseMenu].isOpen = true;
					...
					if (me == Game.instance._player) {
						...
						{
							GameMenu.MenuList[MenuName.InventoryMenu].isOpen = true;
						}
					}
